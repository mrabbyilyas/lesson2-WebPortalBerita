                    ...
                    <div
                        ...
                        <div class="thumbnail-container h-[200px] relative rounded-[20px] overflow-hidden">
                            <div
                                ...
                                <p class="text-xs leading-[18px] font-bold uppercase">{{$item_news->category->name}}</p>
                            </div>
                            <img src="{{Storage::url($item_news->thumbnail)}}" alt="thumbnail photo"
                                class="w-full h-full object-cover" />
                        </div>
                        <div class="flex flex-col gap-[6px]">
                            <h3 class="text-lg leading-[27px] font-bold">{{substr($item_news->name, 0, 50)}}{{strlen($item_news->name) > 50 ? '...' : ''}}</h3>
                            <p class="text-sm leading-[21px] text-[#A3A6AE]">{{$item_news->created_at->format('M d, Y')}}</p>
                        </div>
                    </div>
                </a>
            @empty
